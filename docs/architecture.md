...
Switch diagnosis is intentionally outside the daily `LayoutSpec`. `src/switch_test.rs` generates an ephemeral one-layer keymap under `tmp/`; its dedicated build cache explicitly overrides ZMK's keymap file without replacing either committed projection or trusting a cached devicetree overlay. The flasher targets that distinct artifact explicitly. Normal firmware builds pass their own keymap path, so `just flash left` reliably restores the daily layout.

On macOS, the flashing boundary identifies physical halves by USB serial rather than the shared `NRF52BOOT` label. The first explicit `just flash left` or `just flash right` remembers that serial in ignored `.env`; registration is an implementation detail rather than a separate user workflow. Flashing discovers candidate volumes, validates their UF2 identity, joins each disk's device-tree location to its USB serial, and resolves every serial to a side before copying anything. Unknown, duplicate, or ambiguous devices abort the complete operation rather than allowing a positional guess.
